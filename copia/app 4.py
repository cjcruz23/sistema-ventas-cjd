...
# --- FUNCIÓN NÚCLEO: GENERACIÓN DE PDF CONTRATO ---
...
if choice == "Registrar Cliente":
    ...
    if res_c.data:
        ...
        if st.button("Finalizar Venta y Descargar Contrato"):
            v_ins = supabase.table("ventas").insert({
                ...
                "fecha_venta": str(datetime.now().date()), "costo_compra": costo_compra,
    ...
    if res.data:
        ...
        # --- CONSULTA Y LIMPIEZA DE COLUMNAS ---
        ...
        # Obviamos las columnas técnicas id y venta_id
        columnas_visibles = ["numero_cuota", "fecha_vencimiento", "monto", "pagado", "estado"]
        df_display = df_cartera[columnas_visibles]
        ...
        c2.metric("Total Recaudado", f"${int(pagado_real):,}", delta=f"${int(v_sel['cuota_inicial']):,} Inicial")
        c3.metric("Saldo Pendiente", f"${int(saldo_pendiente):,}", delta_color="inverse")

        st.write("**Detalle de Cuotas Actualizado:**")
        # --- CONFIGURACIÓN DE TABLA SIN IDs ---
        st.dataframe(df_display, column_config={
            "numero_cuota": st.column_config.Column("No.", width="small"),
        ...
        with tab1:
            ...
            if st.button("Registrar Pago y Conciliar"):
                ...
                resto = monto_abono
                # Usamos df_cartera original para la lógica de actualización en DB
                ...
                with open(path_recibo, "rb") as f:
                    st.download_button("📥 Descargar Recibo de Caja", f, file_name=f"Recibo_{v_sel['id']}.pdf")
                st.rerun()
        with tab2:
            ...
            if pagos_hist.data:
                for p in pagos_hist.data:
                    ...
                    if col2.button("Reimprimir", key=f"re_pago_{p['id']}"):
                        ...
                        with open(path_re, "rb") as f:
                            st.download_button("📥 Descargar PDF", f, file_name=f"Recibo_Caja_{p['id']}.pdf")
    else: st.info("No hay ventas para los criterios de búsqueda.")
