...
# --- FUNCIÓN NÚCLEO: GENERACIÓN DE PDF PROFESIONAL (3 PÁGINAS) ---
def generar_pdf_contrato(datos_venta, plan_pagos, cliente, id_venta):
    ...
    # --- PÁGINA 1: CONTRATO DE COMPRAVENTA ---
    pdf.add_page()
    agregar_encabezado()
...
# Agregamos la nueva opción aquí
...
if choice == "Registrar Cliente":
    ...
    st.header("Generar Venta y Contrato")
    ...
    if res_c.data:
        ...
        v1, v2 = st.columns(2)
        with v1:
            art = st.text_input("Artículo/Producto"); precio = st.number_input("Precio Total", min_value=0.0)
        with v2:
            inic = st.number_input("Cuota Inicial", min_value=0.0); n_cuo = st.number_input("Cuotas", min_value=1, value=5)
        sal = precio - inic
        ...
        edit_df = st.data_editor(df_cuo, hide_index=True, use_container_width=True)
        if st.button("Finalizar Venta y Descargar PDF"):
            v_ins = supabase.table("ventas").insert({"cliente_id": dat_c['id'], "producto": art, "monto_total": precio, "cuota_inicial": inic, "fecha_venta": str(datetime.now().date())}).execute()
            ...
            path = generar_pdf_contrato({"producto": art, "monto_total": precio, "cuota_inicial": inic}, edit_df, dat_c, v_id)
    ...
    st.header("Auditoría de Ventas Realizadas")
    ...
    if hist.data:
        for r in hist.data:
            with st.expander(f"{r['fecha_venta']} | {r['clientes']['nombre']} - {r['producto']}"):
                st.write(f"Venta ID: {r['id']} | Total: ${int(r['monto_total']):,}")
...
# --- NUEVO MÓDULO DE CARTERA ---
elif choice == "Control de Cartera":
    st.header("Gestión de Recaudo y Auditoría")
    
    # 1. BUSCADOR INTELIGENTE - SIN ERROR DE PARSEO
    busqueda = st.text_input("🔍 Buscar por nombre o cédula")
    
    # Buscamos primero en la tabla clientes
    ids_clientes = []
    if busqueda:
    ...
    # 2. Consultamos ventas filtrando por esos IDs (o todas si no hay búsqueda)
    query = supabase.table("ventas").select("*, clientes(*)")
    if busqueda:
        if ids_clientes:
            query = query.in_("cliente_id", ids_clientes)
        else:
            query = query.eq("id", -1) # Si buscó y no encontró, forzamos resultado vacío
    ...
    if res.data:
        ...
        # 3. ENTRADA DE DINERO (Con formato de miles)
        monto_abono = st.number_input("Valor del Abono", min_value=0, step=1000, format="%d")
        st.write(f"**Valor a registrar:** {monto_abono:,.0f} Pesos")
        
        # 4. BOTÓN Y LÓGICA DE CONCILIACIÓN
        if st.button("Registrar y Conciliar"):
            ...
            for cuota in cuotas.data:
                ...
                if saldo_cuota > 0:
                    ...
                    nuevo_estado = 'Cancelada' if nuevo_pagado >= cuota['monto'] else 'Abonada'
            ...
            st.success("Pago conciliado correctamente.")
            st.rerun()

        # 5. TABLA DE AUDITORÍA (Con formato de miles)
        cuotas = supabase.table("plan_pagos").select("*").eq("venta_id", v_sel['id']).order("numero_cuota").execute()
        if cuotas.data:
            df_display = pd.DataFrame(cuotas.data)
            for col in ['monto', 'pagado']:
                df_display[col] = df_display[col].apply(lambda x: f"{int(x):,}")
            st.table(df_display[['numero_cuota', 'fecha_vencimiento', 'monto', 'pagado', 'estado']])
    else:
