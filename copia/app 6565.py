...
# Credenciales de su base de datos en Supabase
...
# --- FUNCIÓN NÚCLEO: GENERACIÓN DE PDF CONTRATO COMPLETO ---
...
if choice == "📊 Tablero de Control":
    ...
    if v_res.data and p_res.data:
        ...
        total_c = df_v['costo_compra'].sum() + df_v['gasto_transporte'].sum() + df_v['gasto_papeleria'].sum() + df_v['gasto_otros'].sum()
        ...
        c2.metric("Utilidad Proyectada", f"${int(utilidad):,}")
        ...
        # CORRECCIÓN DE ERROR PANDAS: 'ME' en lugar de 'M'
        ventas_mes = df_v.resample('ME', on='fecha_venta')['monto_total'].sum()
        st.bar_chart(ventas_mes)

        st.subheader("🚩 Alertas de Mora")
        hoy = datetime.now().date()
        df_p['fecha_vencimiento'] = pd.to_datetime(df_p['fecha_vencimiento']).dt.date
        mora = df_p[(df_p['estado'] != 'Cancelada') & (df_p['fecha_vencimiento'] < hoy)].copy()
        
        if not mora.empty:
            st.warning(f"Existen {len(mora)} cuotas vencidas.")
            st.dataframe(mora[["numero_cuota", "fecha_vencimiento", "monto", "pagado"]], use_container_width=True, hide_index=True)
        else:
    ...
    with st.form("cli_form"):
        ...
        with c1:
            ...
            c_cc = st.text_input("Cédula")
        with c2:
            d = st.text_input("Dirección de Residencia")
    ...
    if res_c.data:
        ...
        with v3:
            # Captura de fecha real de negocio
            fecha_real_venta = st.date_input("Fecha Real de la Venta", value=datetime.now().date())

        st.subheader("Costos y Gastos Operativos")
        co1, co2, co3, co4 = st.columns(4)
        with co1: costo_compra = st.number_input("Costo Compra Artículo", min_value=0, step=1000, value=0)
        with co2: transporte = st.number_input("Gasto Transporte", min_value=0, step=1000, value=0)
        with co3: papeleria = st.number_input("Gasto Papelería", min_value=0, step=1000, value=0)
        with co4: otros_gastos = st.number_input("Otros Gastos", min_value=0, step=1000, value=0)
        ...
        sal = precio - inic
        # Proyección de cuotas basada en la fecha real elegida
        df_cuo = pd.DataFrame([
            {
                "Cuota": i+1, 
                "Fecha Vencimiento": (fecha_real_venta + timedelta(days=(i+1)*15)), 
                "Monto": sal/n_cuo
            } for i in range(n_cuo)
        ])
        
        st.write(f"**Proyección del Plan de Pagos (Desde: {fecha_real_venta}):**")
        st.dataframe(df_cuo, column_config={"Monto": st.column_config.NumberColumn("Monto", format="$ %,.0f")}, hide_index=True, use_container_width=True)
        
        if st.button("Finalizar Venta y Descargar Contrato"):
            v_ins = supabase.table("ventas").insert({
                "cliente_id": dat_c['id'], "producto": art, "monto_total": precio, "cuota_inicial": inic, 
                "fecha_venta": str(fecha_real_venta), "costo_compra": costo_compra,
                "gasto_transporte": transporte, "gasto_papeleria": papeleria, "gasto_otros": otros_gastos
            }).execute()
    ...
    st.header("Gestión de Recaudo y Cartera")
    ...
    if busqueda:
        ...
        ids = [c['id'] for c in res_cli.data]
        query = query.in_("cliente_id", ids) if ids else query.eq("id", -1)
    res = query.execute()

    if res.data:
        opciones = {f"{v['clientes']['nombre']} - {v['producto']}": v for v in res.data}
        sel_venta = st.selectbox("Seleccione la venta para gestionar", list(opciones.keys()))
        v_sel = opciones[sel_venta]
        
        cuotas_data = supabase.table("plan_pagos").select("*").eq("venta_id", v_sel['id']).order("numero_cuota").execute()
        df_cartera = pd.DataFrame(cuotas_data.data)
        
        total_venta = v_sel['monto_total']
        pagado_real = df_cartera['pagado'].sum() + v_sel['cuota_inicial']
        saldo_pendiente = total_venta - pagado_real

        st.markdown("### 📊 Estado de Cuenta Actual")
        c1, c2, c3 = st.columns(3)
        c1.metric("Valor Total Venta", f"${int(total_venta):,}")
        c2.metric("Total Recaudado", f"${int(pagado_real):,}")
        c3.metric("Saldo Pendiente", f"${int(saldo_pendiente):,}", delta_color="inverse")

        st.write("**Detalle del Plan de Pagos:**")
        st.dataframe(df_cartera[["numero_cuota", "fecha_vencimiento", "monto", "pagado", "estado"]], 
            column_config={
                "numero_cuota": "No.",
                "monto": st.column_config.NumberColumn("Valor Cuota", format="$ %,.0f"),
                "pagado": st.column_config.NumberColumn("Abonado", format="$ %,.0f")
            }, hide_index=True, use_container_width=True)

        tab1, tab2 = st.tabs(["➕ Registrar Pago", "📜 Histórico"])
        with tab1:
            ...
            if st.button("Procesar Pago"):
                ...
                resto = monto_abono
                for _, cuota in df_cartera.iterrows():
                    if resto <= 0: break
                    ya_pagado = cuota.get('pagado') or 0
                    saldo_cuota = cuota['monto'] - ya_pagado
                    if saldo_cuota > 0:
                        aplicado = min(resto, saldo_cuota)
                        nuevo_total = ya_pagado + aplicado
                        supabase.table("plan_pagos").update({"pagado": nuevo_total, "estado": 'Cancelada' if nuevo_total >= cuota['monto'] else 'Abonada'}).eq("id", cuota['id']).execute()
                        resto -= aplicado
                pr = generar_recibo_caja(v_sel['clientes'], monto_abono, metodo, v_sel['producto'])
                with open(pr, "rb") as f: st.download_button("📥 Descargar Recibo", f, file_name=f"Recibo_{v_sel['id']}.pdf")
                st.rerun()
        with tab2:
            pagos_hist = supabase.table("pagos").select("*").eq("venta_id", v_sel['id']).order("fecha_pago", desc=True).execute()
            for p in pagos_hist.data: st.write(f"📅 {p['fecha_pago']} | ${int(p['monto']):,} | {p['metodo_pago']}")
    ...
    for r in hist.data:
        with st.expander(f"{r['fecha_venta']} | {r['clientes']['nombre']} - {r['producto']}"):
            st.write(f"Venta: ${int(r['monto_total']):,} | Costo Compra: ${int(r['costo_compra']):,}")
            st.write(f"Utilidad: ${int(r['monto_total'] - (r['costo_compra'] + r['gasto_transporte'] + r['gasto_papeleria'] + r['gasto_otros'])):,}")
