...
def generar_recibo_caja(cliente, monto, metodo, producto):
    ...
    # --- AJUSTE ESTÉTICO FINAL DE LA FIRMA ---
    y_actual = recibo.get_y()
    margen_firma = 15
    y_pos_firma = y_actual + margen_firma
    
    if os.path.exists("firma.png"):
        recibo.image("firma.png", x=10, y=y_pos_firma, w=50)
    
    # Aumentamos el valor de 30 a 35 para la línea
    # Aumentamos el valor de 35 a 40 para el texto
    # Esto da un espacio más holgado y elegante
    recibo.line(10, y_pos_firma + 35, 80, y_pos_firma + 35)
    recibo.set_font("Arial", 'B', 10)
    recibo.text(10, y_pos_firma + 40, "Firma Autorizada - CJ&D")    
...
if choice == "Registrar Cliente":
    ...
    if res.data:
        ...
        with tab1:
            ...
            if st.button("Registrar y Conciliar"):
                try:
                    ...
                    for cuota in cuotas.data:
                        ...
                        if saldo_cuota > 0:
                            ...
                            supabase.table("plan_pagos").update({
                                "pagado": nuevo_pagado,
                                "estado": 'Cancelada' if nuevo_pagado >= cuota['monto'] else 'Abonada'
                    ...
                    st.success("Pago registrado y conciliado correctamente.")
                    st.rerun()
        ...
        if cuotas.data:
            ...
            for col in ['monto', 'pagado']: df_display[col] = df_display[col].apply(lambda x: f"{int(x):,}")
